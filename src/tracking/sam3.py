...
def _run_single_video(cfg, run_dir: Path, config_path: Path | None = None):
    ...
    if reuse_chunk_info:
        ...
        reused_frame_pairs, reused_prompt_points = load_chunks_from_chunk_info(
            reuse_run_path
        )
        logger.info(f"Loaded {len(reused_frame_pairs)} chunks from previous run")
        if reused_prompt_points:
            logger.info(
                f"  {len(reused_prompt_points)} chunk(s) have stored prompt_points"
            )
    ...
    if use_manual_chunking:
        ...
        tracker_overrides = (
            set(reused_prompt_points.keys()) if reused_prompt_points else None
        )
        chunks = build_manual_chunks(
            manual_chunk_frames, tracker_override_indices=tracker_overrides
        )
    ...
    for chunk_idx, (start_idx, end_idx, chunk_type) in enumerate(chunks):
        ...
        if grounding_enabled:
            ...
            if grounding_results_path.exists():
                ...
                gs_chunk_df = gs_chunk_df[
                    ~gs_chunk_df.index.duplicated(keep="last")
                ].sort_index()
                del gs_existing_df
            ...
            logger.info(
                f"  [manual] Using reused prompt_points ({len(all_prompt_points)} objects)"
            )
...
def run(cfg, config_path: str | Path):
    """Entry point called by the launcher after env vars are configured."""
